    ...
    <div ref={containerRef} className="relative min-h-screen bg-void text-mercury selection:bg-gold-leaf selection:text-void overflow-hidden">
      ...
      <nav className="fixed top-0 left-0 w-full z-50 px-8 py-8 md:px-14 flex justify-between items-center mix-blend-difference pointer-events-none">
        ...
        <div className="hidden lg:flex gap-16 pointer-events-auto items-center">
          {["Philosophy", "Method", "Results"].map((item) => (
        ...
        >
          <motion.div initial="hidden" animate="visible" variants={stagger} className="space-y-12">
            ...
            <motion.h1 className="text-[13vw] font-serif leading-[0.8] tracking-tighter text-reveal-mask relative mix-blend-difference">
              ...
              <span className="block italic text-transparent bg-clip-text bg-gradient-to-r from-gold-leaf via-white to-gold-leaf pr-10 ml-[0.5em]">Reality.</span>
            </motion.h1>
              ...
              <div className="flex gap-[2px] h-12 items-end">
                 {[...Array(24)].map((_, i) => (
                   <motion.div 
                     ...
                     className="w-1 bg-white/20 group-hover/hud:bg-gold-leaf/60 transition-colors duration-500 rounded-t-sm"
                   />
                 ))}
              </div>
           </div>
                    ...
                    <div className="w-full h-full flex items-end justify-between gap-1">
                      {[40, 65, 45, 80, 55, 90, 70, 95].map((h, i) => (
                        <motion.div 
                          ...
                          className="w-full bg-white/10 hover:bg-gold-leaf transition-colors duration-300 rounded-t-sm"
                        />
                      ))}
                    </div>
                 </div>
      ...
      <section className="py-24 space-y-48 relative z-20">
        ...
        <div className="grid lg:grid-cols-2 gap-24 items-center px-8 md:px-20 lg:px-40">
          <motion.div 
            initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}
            className="relative aspect-[4/5] bg-black border border-white/5 overflow-hidden group"
          >
            {/* Image Layer */}
            ...
            <motion.div 
              animate={{ top: ["-10%", "110%"] }}
              transition={{ duration: 4, repeat: Infinity, ease: "linear" }}
              className="absolute left-0 right-0 h-[1px] bg-white/60 z-30 pointer-events-none shadow-[0_0_15px_rgba(255,255,255,0.5)]"
            />
            ...
            <div className="absolute top-10 left-10 z-40 space-y-4">
               <div className="flex items-center gap-3">
                  ...
                  <span className="text-[9px] font-mono uppercase tracking-[0.5em] text-white/60">Neural Link: ACTIVE</span>
               </div>
               <div className="w-48 h-[1px] bg-white/10 relative overflow-hidden">
                  <motion.div 
                    animate={{ x: ["-100%", "100%"] }}
                    transition={{ duration: 2, repeat: Infinity, ease: "linear" }}
                    className="absolute inset-0 bg-white/40 w-1/4"
                  />
               </div>
            </div>
            ...
            <div className="absolute bottom-10 right-10 z-40 opacity-40">
               <svg width="80" height="80" viewBox="0 0 100 100" className="rotate-[-90deg]">
                  <circle cx="50" cy="50" r="45" fill="none" stroke="currentColor" strokeWidth="0.5" className="text-white/20" />
                  <motion.circle 
                    cx="50" cy="50" r="45" 
                  ...
                  />
                  <text x="50" y="55" textAnchor="middle" className="text-[10px] font-mono fill-white/60 uppercase tracking-tighter" transform="rotate(90 50 50)">VRTX</text>
               </svg>
            </div>
          </motion.div>
        ...
        <div className="grid lg:grid-cols-2 gap-20 items-center px-8 md:px-20 lg:px-40">
          <motion.div 
            initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}
            className="lg:order-2 relative aspect-[4/5] bg-black border border-white/5 overflow-hidden group"
          >
            ...
            <div className="absolute inset-0 border-[20px] border-black/40 backdrop-blur-[2px] m-8 pointer-events-none" />
            ...
            <div className="absolute bottom-12 right-12 text-right space-y-2 font-mono text-[9px] uppercase tracking-widest text-white/40">
          ...
          >
            ...
            <div className="pt-8 grid grid-cols-2 gap-8 border-t border-white/10 w-full">
              <div>
                ...
                <div className="text-xl font-serif text-white italic">Hypertrophy</div>
              </div>
              <div>
                <div className="text-[9px] font-mono text-gold-leaf uppercase tracking-widest mb-1">Method</div>
                <div className="text-xl font-serif text-white italic">Heavy Duty</div>
              </div>
            </div>
          </motion.div>
        </div>
        ...
        <div className="max-w-[1400px] mx-auto flex flex-col md:flex-row justify-between items-start gap-16 md:items-center">
          ...
          <div className="flex gap-20">
             {["Instagram", "Twitter"].map(link => (
               <a key={link} href="#" className="text-[10px] font-bold uppercase tracking-[0.3em] text-white/20 hover:text-gold-leaf transition-colors relative group">
                 {link}
                 <span className="absolute -bottom-1 left-0 w-0 h-[1px] bg-gold-leaf transition-all duration-300 group-hover:w-full" />
               </a>
             ))}
             <Link href="/journal" className="text-[10px] font-bold uppercase tracking-[0.3em] text-white/20 hover:text-gold-leaf transition-colors relative group">
                Journal
                <span className="absolute -bottom-1 left-0 w-0 h-[1px] bg-gold-leaf transition-all duration-300 group-hover:w-full" />
             </Link>
          </div>
